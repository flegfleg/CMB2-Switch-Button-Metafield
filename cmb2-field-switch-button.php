...
add_action('cmb2_render_switch', 'cmb2_switch', 10, 5);
...
function cmb2_switch( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
			...
			$switch .= '<input '.$conditional_value.$conditional_id.' type="radio" id="' . $field->args['_id'] . '1" value="1"  '. ($escaped_value == 1 ? 'checked="checked"' : '') . ' name="' . esc_attr($field->args['_name']) . '" />
				...
				<label for="' . $field->args['_id'] . '2" class="cmb2-disable '.(($escaped_value == '' || $escaped_value == 0)?'selected':'').' ' . (( $field->args['disabled_warning'] == 1) ?'warning' :'' ) .'"><span>'.$label_off.'</span></label>
